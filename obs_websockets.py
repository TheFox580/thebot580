...
from websockets_auth import WEBSOCKET_HOST, WEBSOCKET_PASSWORD, WEBSOCKET_PORT
...
class OBSWebsocketsManager:
    ...
    def get_scene(
        ...
        self.ws.call(requests.GetCurrentProgramScene())
...
if __name__ == "__main__":
    ...
    rotation = obswebsockets_manager.get_source_transform("Scene 2", "Chat_Image")[
        "rotation"
    ]
    print(rotation)

    for _ in range(5):
        new_transform = {"rotation": rotation + 5}
        obswebsockets_manager.set_source_transform(
            "Scene 2", "Chat_Image", new_transform
        )
        time.sleep(0.01)
        rotation = obswebsockets_manager.get_source_transform("Scene 2", "Chat_Image")[
            "rotation"
