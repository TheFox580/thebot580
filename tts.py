...
class TTSManager:
    def __init__(self, tts_voice: str):
        try:
            ...
            exit(
                "Ooops! You forgot to set AZURE_TTS_KEY or AZURE_TTS_REGION or AZURE_TTS_ENDPOINT in your environment!"
            )
        self.azure_speechconfig.speech_synthesis_voice_name = tts_voice
        self.azure_synthesizer = speechsdk.SpeechSynthesizer(
            speech_config=self.azure_speechconfig, audio_config=None
        )
    ...
    def text_to_speech(self, input: str):
        ...
        output = os.path.join(
            ...
            f"_Msg{str(hash(input))}-{str(hash(self.azure_speechconfig.speech_synthesis_voice_name))}-{str(hash('friendly'))}.wav",
        )
